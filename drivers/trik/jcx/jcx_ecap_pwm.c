...
static struct resource trik_jcx_ecap0_resource[] = {
	{
...
static struct resource trik_jcx_ecap1_resource[] = {
	{
...
static struct resource trik_jcx_ecap2_resource[] = {
	{
...
int init_module(void)
{
	...
	jc0_pd = platform_device_register_simple("ecap_cap",0,trik_jcx_ecap0_resource,ARRAY_SIZE(trik_jcx_ecap0_resource));
	if (IS_ERR(jc0_pd)) {
		ret = -ENOMEM;
		goto exit_pd_registor_ecap0;
	}
	jc1_pd = platform_device_register_simple("ecap_cap",1,trik_jcx_ecap1_resource,ARRAY_SIZE(trik_jcx_ecap1_resource));
	if (IS_ERR(jc1_pd)) {
		ret = -ENOMEM;
		goto exit_pd_registor_ecap1;
	}
	jc2_pd = platform_device_register_simple("ecap_cap",2,trik_jcx_ecap2_resource,ARRAY_SIZE(trik_jcx_ecap2_resource));
	if (IS_ERR(jc2_pd)) {
		ret = -ENOMEM;
